...
// Verifica se um vetor contém apenas variáveis inteiras, e devolve endereço da maior ou da menor variável que não é inteira.
int localiza_real_custoso(float vetor[TAMANHO], float vetor_custo[TAMANHO], int num_colunas, int minimiza, int *real){
    int i;
    float resto;
    float extremo;
    int endereco = num_colunas;
    if(minimiza == 0){
        extremo = -INFINITO;
        for(i = 0; i < num_colunas; i++){
            resto = vetor[i] - floor(vetor[i]);
            if(resto > 0.0001){ // Aqui dá pra colocar grau de precisão, tipo maior que 0.0001, por exemplo.
                (*real)++;
                if(vetor_custo[i] > extremo){
                    extremo = vetor_custo[i];
                    endereco = i;
                }
            }
        }
...
int branch_bound(float matriz_A[TAMANHO][TAMANHO], int vetor_relacional[TAMANHO], float vetor_b[TAMANHO], int minimiza, float vetor_custo[TAMANHO], vertice *resultados[TAMANHO], int num_linhas, int num_colunas, vertice *sol_otima, FILE *arqtxt){
    ...
    int infactivel = 0;
    int conta_pilha = 0;
    conteudo *pilha = (conteudo *) malloc(sizeof(conteudo));
    conteudo *umProblema = (conteudo *) malloc(sizeof(conteudo));

    cria_pilha_vazia(pilha);
    num_no = 0;
    ...
    while(conta_pilha > 0){
        ...
        if(infactivel == 0){ // Ou seja, tem solução.
            ...
            variavel = localiza_real_custoso((resultados[num_no])->vetor_x, vetor_custo, num_colunas, minimiza, &tem_real);
            ...
            if(tem_real == 0){
                ...
                if((resultados[num_no])->valor_Z != (resultados[(resultados[num_no])->pai])->valor_Z && (resultados[num_no])->vetor_x[variavel] != (resultados[(resultados[num_no])->pai])->vetor_x[variavel]){
                    if((minimiza == 0 && (resultados[num_no])->valor_Z > sol_otima->valor_Z) || (minimiza == 1 && (resultados[num_no])->valor_Z < sol_otima->valor_Z)){
                        ...
                        (resultados[conta_nos + 2])->pai = num_no;
                        (resultados[conta_nos + 2])->no = conta_nos + 2;
                        ...
                        (resultados[conta_nos])->pai = num_no;
                        (resultados[conta_nos])->no = conta_nos;
