...
void enfileira(fila *umaFila, conteudo *umProblema, int *tamanho_fila){
    ...
    *aux = *umProblema;
    ...
    umaFila->fim->proximo = aux; // faz com que o último nó da fila tenha aux como o seu próximo nó (aqui de fato aux é inserido no fim fila).
...
// ADAPTAR ESTA FUNÇÃO, POIS APESAR DE FUNCIONAR, NÃO ESTÁ ORDENANDO EXATAMENTE COMO DEVERIA.
// Função que enfileira decrescentemente (se max) de acordo com Z.
void enfileira_ordenado(fila *umaFila, conteudo *umProblema, int *tamanho_fila, vertice *resultados[TAMANHO], int num_no, int minimiza){
    conteudo *aux = (conteudo*) malloc(sizeof(conteudo));
    *aux = *umProblema;
    conteudo *fim_fila = (conteudo*) malloc(sizeof(conteudo));
    ...
    if(*tamanho_fila == 0){ // Caso seja o primeiro elemento, faz com que os ponteiros inicio e fim apontem para ele.
        ...
        fim_fila = umaFila->fim;
        umaFila->fim = umaFila->inicio;
        if(minimiza == 0){
            while(umaFila->fim->proximo != NULL && (resultados[num_no])->valor_Z <= (resultados[(resultados[umaFila->fim->proximo->no])->pai])->valor_Z){
                umaFila->fim = umaFila->fim->proximo;
            }
        }
        else{
            while(umaFila->fim->proximo != NULL && (resultados[num_no])->valor_Z >= (resultados[(resultados[umaFila->fim->proximo->no])->pai])->valor_Z){
        ...
        aux->proximo = umaFila->fim->proximo;
        umaFila->fim->proximo = aux;
        umaFila->fim = fim_fila;
    }
    (*tamanho_fila)++;
    printf("\n\n\nElemento inserido com sucesso");
}
            ...
            if(tem_real == 0){
                ...
                if((resultados[num_no])->valor_Z != (resultados[(resultados[num_no])->pai])->valor_Z && (resultados[num_no])->vetor_x[variavel] != (resultados[(resultados[num_no])->pai])->vetor_x[variavel]){
                    if((minimiza == 0 && (resultados[num_no])->valor_Z > sol_otima->valor_Z) || (minimiza == 1 && (resultados[num_no])->valor_Z < sol_otima->valor_Z)){
                        ...
                        enfileira_ordenado(umaFila, umProblema, &conta_pilha, resultados, num_no, minimiza);
                        conta_nos++;
                        //conta_pilha++;
